...
# This notebook details the first iteration of using Multinomial Naive Bayes to identify suspicious employee activity. For ease of development, all classifiers are currently built and trained in this notebook. In the future, a separate notebook will be created for each classifier.
# 
# #### Changelog
# 
# *Version: 1 (Current)*
...
# Initialise and fit Multinomial NB classifiers

pc_clf = MultinomialNB(alpha=0.00001)
pc_clf.fit(pc_train_vectors, pc_train['label'])

building_clf = MultinomialNB(alpha=0.00001)
building_clf.fit(building_train_vectors, building_train['label'])

proxy_clf = MultinomialNB(alpha=0.00001)
...
pc_train_f1 = f1_score(pc_train['label'].tolist(), pc_train_preds)
pc_train_f1_weighted = f1_score(pc_train['label'].tolist(), pc_train_preds, average='weighted')
pc_train_confusion = confusion_matrix(pc_train['label'].tolist(), pc_train_preds, labels=cm_labels)
...
print('Results for Multinomial NB on pc_train:')
...
building_train_f1 = f1_score(building_train['label'].tolist(), building_train_preds)
building_train_f1_weighted = f1_score(building_train['label'].tolist(), building_train_preds, average='weighted')
building_train_confusion = confusion_matrix(building_train['label'].tolist(), building_train_preds, labels=cm_labels)
...
print('Results for Multinomial NB on building_train:')
...
proxy_train_f1 = f1_score(proxy_train['label'].tolist(), proxy_train_preds)
proxy_train_f1_weighted = f1_score(proxy_train['label'].tolist(), proxy_train_preds, average='weighted')
proxy_train_confusion = confusion_matrix(proxy_train['label'].tolist(), proxy_train_preds, labels=cm_labels)
...
print('Results for Multinomial NB on proxy_train:')

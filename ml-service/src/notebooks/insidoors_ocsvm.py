...
# This notebook details the third iteration of using One-class SVM to identify suspicious employee activity. For ease of development, all classifiers are currently built and trained in this notebook. When integrating with the main application, a separate executable script should be created for each classifier.
# 
# #### Changelog
# 
# *Version: 3 (Current)*
...
display(building_X_train_normal)
...
# Initialise and fit One-class SVM for PC Access logs

pc_clf = GridSearchCV(OneClassSVM(),
                      param_grid=params,
                      scoring=pc_scorer,
                      cv=cv,
                      verbose=3)
...
# Initialise and fit One-class SVM for Building Access logs

building_clf = GridSearchCV(OneClassSVM(),
                            param_grid=params,
...
# Initialise and fit One-class SVM for Proxy logs

proxy_clf = GridSearchCV(OneClassSVM(),
                         param_grid=params,
...
if useStandardScaler:
    PC_CLF = path + 'pc_ocsvm_scale.joblib'
    BUILDING_CLF = path + 'building_ocsvm_scale.joblib'
    PROXY_CLF = path + 'proxy_ocsvm_scale.joblib'
else:
    PC_CLF = path + 'pc_ocsvm_norm.joblib'
    BUILDING_CLF = path + 'building_ocsvm_norm.joblib'
    PROXY_CLF = path + 'proxy_ocsvm_norm.joblib'

...
# * Implemented XGBoost with categorical data
...
# Define function to append cyclical features to dataframe

def create_cyclical_features(df):
    cf = CyclicalFeatures(drop_original=True)
    cyclical_cols = ['access_month', 'access_day', 'access_weekday', 'access_hour', 'access_minute', 'access_second']
    
    cyclical = cf.fit_transform(df[cyclical_cols])
    df = df.drop(labels=cyclical_cols, axis=1)
    df = pd.concat([df, cyclical], axis=1)
...
# Append cyclical features to dataframes

pc_df = create_cyclical_features(pc_df)
building_df = create_cyclical_features(building_df)
proxy_df = create_cyclical_features(proxy_df)


# In[ ]:


# Specify datatype of categorical features

pc_df = pc_df.astype({'log_on_off': 'category',
                      'machine_location': 'category',
                      'working_hours': 'category',
                      'user_location': 'category',
                      'terminated': 'category'})

building_df = building_df.astype({'direction': 'category',
                                  'status': 'category',
                                  'office_location': 'category',
                                  'terminated': 'category'})

proxy_df = proxy_df.astype({'category': 'category'})
...
pc_clf = GridSearchCV(XGBClassifier(enable_categorical=True,
                                    scale_pos_weight=pc_weight,
                                    eval_metric='auc'),
                      param_grid=params,
                      scoring='roc_auc',
                      cv=cv,
...
pc_clf.fit(pc_X_train.drop(labels='label', axis=1), pc_y_train)
...
pc_test_scores = pc_clf.predict_proba(pc_X_test.drop(labels='label', axis=1))[:,1]
pc_test_preds = pc_clf.predict(pc_X_test.drop(labels='label', axis=1))
...
building_clf = GridSearchCV(XGBClassifier(enable_categorical=True,
                                          scale_pos_weight=building_weight,
                                          eval_metric='auc'),
                            param_grid=params,
                            scoring='roc_auc',
                            cv=cv,
...
building_clf.fit(building_X_train.drop(labels='label', axis=1), building_y_train)
...
building_test_scores = building_clf.predict_proba(building_X_test.drop(labels='label', axis=1))[:,1]
building_test_preds = building_clf.predict(building_X_test.drop(labels='label', axis=1))
...
proxy_clf = GridSearchCV(XGBClassifier(enable_categorical=True,
                                       scale_pos_weight=proxy_weight,
                                       eval_metric='auc'),
                         param_grid=params,
                         scoring='roc_auc',
                         cv=cv,
...
proxy_clf.fit(proxy_X_train.drop(labels='label', axis=1), proxy_y_train)
...
proxy_test_scores = proxy_clf.predict_proba(proxy_X_test.drop(labels='label', axis=1))[:,1]
proxy_test_preds = proxy_clf.predict(proxy_X_test.drop(labels='label', axis=1))
...
PC_CLF = path + 'pc_xgb_categorical.joblib'
BUILDING_CLF = path + 'building_xgb_categorical.joblib'
PROXY_CLF = path + 'proxy_xgb_categorical.joblib'
...
pc_unseen = pc_unseen.drop(labels='label', axis=1)
...
building_unseen = building_unseen.drop(labels='label', axis=1)
...
proxy_unseen = proxy_unseen.drop(labels='label', axis=1)
...
pc_unseen = pc_unseen.astype({'log_on_off': 'category',
                              'machine_location': 'category',
                              'working_hours': 'category',
                              'user_location': 'category',
                              'terminated': 'category'})

building_unseen = building_unseen.astype({'direction': 'category',
                                          'status': 'category',
                                          'office_location': 'category',
                                          'terminated': 'category'})

proxy_unseen = proxy_unseen.astype({'category': 'category'})
...
pc_unseen_scores = pc_clf_loaded.predict_proba(pc_unseen)[:,1]
pc_unseen_preds = pc_clf_loaded.predict(pc_unseen)
...
building_unseen_scores = building_clf_loaded.predict_proba(building_unseen)[:,1]
building_unseen_preds = building_clf_loaded.predict(building_unseen)
...
proxy_unseen_scores = proxy_clf_loaded.predict_proba(proxy_unseen)[:,1]
proxy_unseen_preds = proxy_clf_loaded.predict(proxy_unseen)

...
# This notebook details the second iteration of using Isolation Forest to identify suspicious employee activity. For ease of development, all classifiers are currently built and trained in this notebook. When integrating with the main application, a separate executable script should be created for each classifier.
# 
# #### Changelog
# 
# *Version: 2 (Current)*
...
# ### Train anomaly detection classifier with Isolation Forest
...
pc_clf = GridSearchCV(IsolationForest(),
                      param_grid=params,
                      scoring=pc_scorer,
                      cv=cv,
                      verbose=3)
...
building_clf = GridSearchCV(IsolationForest(),
                            param_grid=params,
...
# Initialise and fit Isolation Forest for Proxy logs

proxy_clf = GridSearchCV(IsolationForest(),
                         param_grid=params,
...
if useStandardScaler:
    PC_CLF = path + 'pc_if_scale.joblib'
    BUILDING_CLF = path + 'building_if_scale.joblib'
    PROXY_CLF = path + 'proxy_if_scale.joblib'
else:
    PC_CLF = path + 'pc_if_norm.joblib'
    BUILDING_CLF = path + 'building_if_norm.joblib'
    PROXY_CLF = path + 'proxy_if_norm.joblib'
